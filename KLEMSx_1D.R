Data1list <- split(KLEMS_1, KLEMS_1$Industry)
dlist1 <- lapply(seq_along(Data1list), function(x) as.data.frame(Data1list[[x]])) 

lapply(seq_along(dlist1), function(x) {
  assign(paste0("Data1.", x), Data1list[[x]], envir=.GlobalEnv)
}
)
...
R2_1D <- data.frame(matrix(NA, ncol=13, nrow=35))
names(R2_1D) <- c("Industry", "R2_Eq1", "R2_Eq2", "R2_Eq3", "R2_Eq4", "Bg_Eq1", "Bg_Eq2",
                  "Bg_Eq3", "Bg_Eq4", "Mono", "Non-Mono", "Concavity", "Non-Concavity")

Coef_1D <- data.frame(matrix(NA, ncol=33, nrow=35))
names(Coef_1D) <- c("Industry",
...
Err_1D <- data.frame(matrix(NA, ncol=33, nrow=35))
names(Err_1D) <- c("Industry",
                   "eq1_(Intercept)", "eq1_lnK", "eq1_lnL", "eq1_lnE", "eq1_lnM", "eq1_lnY", "eq1_t", "eq1_dK",
                   "eq2_(Intercept)", "eq2_lnK", "eq2_lnL", "eq2_lnE", "eq2_lnM", "eq2_lnY", "eq2_t", "eq2_dL",
                   "eq3_(Intercept)", "eq3_lnK", "eq3_lnL", "eq3_lnE", "eq3_lnM", "eq3_lnY", "eq3_t", "eq3_dE",
                   "eq4_(Intercept)", "eq4_lnK", "eq4_lnL", "eq4_lnE", "eq4_lnM", "eq4_lnY", "eq4_t", "eq4_dM")

pval_1D <- data.frame(matrix(NA, ncol=33, nrow=35))
names(pval_1D) <- c("Industry",
                    "eq1_(Intercept)", "eq1_lnK", "eq1_lnL", "eq1_lnE", "eq1_lnM", "eq1_lnY", "eq1_t", "eq1_dK",
                    "eq2_(Intercept)", "eq2_lnK", "eq2_lnL", "eq2_lnE", "eq2_lnM", "eq2_lnY", "eq2_t", "eq2_dL",
                    "eq3_(Intercept)", "eq3_lnK", "eq3_lnL", "eq3_lnE", "eq3_lnM", "eq3_lnY", "eq3_t", "eq3_dE",
                    "eq4_(Intercept)", "eq4_lnK", "eq4_lnL", "eq4_lnE", "eq4_lnM", "eq4_lnY", "eq4_t", "eq4_dM")

Elast_1D <- data.frame(matrix(NA, ncol=26, nrow=35))
names(Elast_1D) <- c("Industry", "e_KK", "e_KL", "e_KE", "e_KM", "e_KS", "e_LK", "e_LL", "e_LE", "e_LM", "e_LS",
                     "e_EK", "e_EL", "e_EE", "e_EM", "e_ES", "e_MK", "e_ML", "e_ME", "e_MM", "e_MS",
                     "e_SK", "e_SL", "e_SE", "e_SM", "e_SS")

dElast_1D <- data.frame(matrix(NA, ncol=26, nrow=35))
names(dElast_1D) <- c("Industry", "de_KK", "de_KL", "de_KE", "de_KM", "de_KS",
                      "de_LK", "de_LL", "de_LE", "de_LM", "de_LS", "de_EK", "de_EL", "de_EE", "de_EM", "de_ES",
                      "de_MK", "de_ML", "de_ME", "de_MM", "de_MS", "de_SK", "de_SL", "de_SE", "de_SM", "de_SS")

for(i in c(1:35)){ #1:61
  data <- get(paste0("Data1.",i))
  ...
  if (success == FALSE | any(R2[1:4] < 0)){
    ...
    for (j in 1:8){
      R2_1D[i,j+1] <- R2[j] #(summary(system_eqn)[[10]][[1]])$r.squared
    }
    
    for (j in 1:8){
      Coef_1D[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,1]
      Coef_1D[i,j+9] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,1]
      Coef_1D[i,j+17] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,1]
      Coef_1D[i,j+25] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,1]
      Err_1D[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,2]
      Err_1D[i,j+9] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,2]
      Err_1D[i,j+17] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,2]
      Err_1D[i,j+25] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,2]
      pval_1D[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,4]
      pval_1D[i,j+9] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,4]
      pval_1D[i,j+17] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,4]
      pval_1D[i,j+25] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,4]
    }
    ...
    for (j in 1:25){
      Elast_1D[i,j+1] <- Elast[j]
      dElast_1D[i,j+1] <- dElast[j]
    }
  }
}

write.csv(R2_1D, "R2_1D.csv")
write.csv(Elast_1D, "Elast_1D.csv")
write.csv(dElast_1D, "dElast_1D.csv")

write.csv(Coef_1D, "Coef_1D.csv")
write.csv(Err_1D, "Err_1D.csv")
write.csv(pval_1D, "pval_1D.csv")

rm(list = ls()[grep("^Data1.", ls())])
rm(list = ls()[grep("^Agg1.", ls())])
rm(dlist1)
