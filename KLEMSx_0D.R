Data0list <- split(KLEMS_0, KLEMS_0$Industry)
dlist0 <- lapply(seq_along(Data0list), function(x) as.data.frame(Data0list[[x]])) 

lapply(seq_along(dlist0), function(x) {
  assign(paste0("Data0.", x), Data0list[[x]], envir=.GlobalEnv)
}
)
...
R2_0D <- data.frame(matrix(NA, ncol=13, nrow=13))
names(R2_0D) <- c("Industry", "R2_Eq1", "R2_Eq2", "R2_Eq3", "R2_Eq4", "Bg_Eq1", "Bg_Eq2",
                  "Bg_Eq3", "Bg_Eq4", "Mono", "Non-Mono", "Concavity", "Non-Concavity")

Coef_0D <- data.frame(matrix(NA, ncol=33, nrow=13))
names(Coef_0D) <- c("Industry",
...
Err_0D <- data.frame(matrix(NA, ncol=33, nrow=13))
names(Err_0D) <- c("Industry",
                   "eq1_(Intercept)", "eq1_lnK", "eq1_lnL", "eq1_lnE", "eq1_lnM", "eq1_lnY", "eq1_t", "eq1_dK",
                   "eq2_(Intercept)", "eq2_lnK", "eq2_lnL", "eq2_lnE", "eq2_lnM", "eq2_lnY", "eq2_t", "eq2_dL",
                   "eq3_(Intercept)", "eq3_lnK", "eq3_lnL", "eq3_lnE", "eq3_lnM", "eq3_lnY", "eq3_t", "eq3_dE",
                   "eq4_(Intercept)", "eq4_lnK", "eq4_lnL", "eq4_lnE", "eq4_lnM", "eq4_lnY", "eq4_t", "eq4_dM")

pval_0D <- data.frame(matrix(NA, ncol=33, nrow=13))
names(pval_0D) <- c("Industry",
                    "eq1_(Intercept)", "eq1_lnK", "eq1_lnL", "eq1_lnE", "eq1_lnM", "eq1_lnY", "eq1_t", "eq1_dK",
                    "eq2_(Intercept)", "eq2_lnK", "eq2_lnL", "eq2_lnE", "eq2_lnM", "eq2_lnY", "eq2_t", "eq2_dL",
                    "eq3_(Intercept)", "eq3_lnK", "eq3_lnL", "eq3_lnE", "eq3_lnM", "eq3_lnY", "eq3_t", "eq3_dE",
                    "eq4_(Intercept)", "eq4_lnK", "eq4_lnL", "eq4_lnE", "eq4_lnM", "eq4_lnY", "eq4_t", "eq4_dM")

Elast_0D <- data.frame(matrix(NA, ncol=26, nrow=13))
names(Elast_0D) <- c("Industry", "e_KK", "e_KL", "e_KE", "e_KM", "e_KS", "e_LK", "e_LL", "e_LE", "e_LM", "e_LS",
                     "e_EK", "e_EL", "e_EE", "e_EM", "e_ES", "e_MK", "e_ML", "e_ME", "e_MM", "e_MS",
                     "e_SK", "e_SL", "e_SE", "e_SM", "e_SS")

dElast_0D <- data.frame(matrix(NA, ncol=26, nrow=13))
names(dElast_0D) <- c("Industry", "de_KK", "de_KL", "de_KE", "de_KM", "de_KS",
                      "de_LK", "de_LL", "de_LE", "de_LM", "de_LS", "de_EK", "de_EL", "de_EE", "de_EM", "de_ES",
                      "de_MK", "de_ML", "de_ME", "de_MM", "de_MS", "de_SK", "de_SL", "de_SE", "de_SM", "de_SS")

for(i in c(1:13)){ #1:61
  data <- get(paste0("Data0.",i))
  ...
  if (success == FALSE | any(R2[1:4] < 0)){
    ...
    for (j in 1:8){
      R2_0D[i,j+1] <- R2[j] #(summary(system_eqn)[[10]][[1]])$r.squared
    }
    
    for (j in 1:8){
      Coef_0D[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,1]
      Coef_0D[i,j+9] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,1]
      Coef_0D[i,j+17] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,1]
      Coef_0D[i,j+25] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,1]
      Err_0D[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,2]
      Err_0D[i,j+9] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,2]
      Err_0D[i,j+17] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,2]
      Err_0D[i,j+25] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,2]
      pval_0D[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,4]
      pval_0D[i,j+9] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,4]
      pval_0D[i,j+17] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,4]
      pval_0D[i,j+25] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,4]
    }
    ...
    for (j in 1:25){
      Elast_0D[i,j+1] <- Elast[j]
      dElast_0D[i,j+1] <- dElast[j]
    }
  }
}

write.csv(R2_0D, "R2_0D.csv")
write.csv(Elast_0D, "Elast_0D.csv")
write.csv(dElast_0D, "dElast_0D.csv")

write.csv(Coef_0D, "Coef_0D.csv")
write.csv(Err_0D, "Err_0D.csv")
write.csv(pval_0D, "pval_0D.csv")

rm(list = ls()[grep("^Data0.", ls())])
rm(list = ls()[grep("^Agg0.", ls())])
rm(dlist0)
