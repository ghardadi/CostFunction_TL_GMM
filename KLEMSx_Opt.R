Optlist <- split(KLEMS_Opt, KLEMS_Opt$Industry)
dlistopt <- lapply(seq_along(Optlist), function(x) as.data.frame(Optlist[[x]])) 

lapply(seq_along(dlistopt), function(x) {
  assign(paste0("Data5.", x), Optlist[[x]], envir=.GlobalEnv)
}
)
...
R2_Opt <- data.frame(matrix(NA, ncol=13, nrow=100))
names(R2_Opt) <- c("Industry", "R2_Eq1", "R2_Eq2", "R2_Eq3", "R2_Eq4", "Bg_Eq1", "Bg_Eq2",
                   "Bg_Eq3", "Bg_Eq4", "Mono", "Non-Mono", "Concavity", "Non-Concavity")

Coef_Opt <- data.frame(matrix(NA, ncol=33, nrow=100))
names(Coef_Opt) <- c("Industry",
...
Err_Opt <- data.frame(matrix(NA, ncol=33, nrow=100))
names(Err_Opt) <- c("Industry",
                   "eq1_(Intercept)", "eq1_lnK", "eq1_lnL", "eq1_lnE", "eq1_lnM", "eq1_lnY", "eq1_t", "eq1_dK",
                   "eq2_(Intercept)", "eq2_lnK", "eq2_lnL", "eq2_lnE", "eq2_lnM", "eq2_lnY", "eq2_t", "eq2_dL",
                   "eq3_(Intercept)", "eq3_lnK", "eq3_lnL", "eq3_lnE", "eq3_lnM", "eq3_lnY", "eq3_t", "eq3_dE",
                   "eq4_(Intercept)", "eq4_lnK", "eq4_lnL", "eq4_lnE", "eq4_lnM", "eq4_lnY", "eq4_t", "eq4_dM")

pval_Opt <- data.frame(matrix(NA, ncol=33, nrow=100))
names(pval_Opt) <- c("Industry",
                    "eq1_(Intercept)", "eq1_lnK", "eq1_lnL", "eq1_lnE", "eq1_lnM", "eq1_lnY", "eq1_t", "eq1_dK",
                    "eq2_(Intercept)", "eq2_lnK", "eq2_lnL", "eq2_lnE", "eq2_lnM", "eq2_lnY", "eq2_t", "eq2_dL",
                    "eq3_(Intercept)", "eq3_lnK", "eq3_lnL", "eq3_lnE", "eq3_lnM", "eq3_lnY", "eq3_t", "eq3_dE",
                    "eq4_(Intercept)", "eq4_lnK", "eq4_lnL", "eq4_lnE", "eq4_lnM", "eq4_lnY", "eq4_t", "eq4_dM")

Elast_Opt <- data.frame(matrix(NA, ncol=26, nrow=100))
names(Elast_Opt) <- c("Industry", "e_KK", "e_KL", "e_KE", "e_KM", "e_KS", "e_LK", "e_LL", "e_LE", "e_LM", "e_LS",
                     "e_EK", "e_EL", "e_EE", "e_EM", "e_ES", "e_MK", "e_ML", "e_ME", "e_MM", "e_MS",
                     "e_SK", "e_SL", "e_SE", "e_SM", "e_SS")

dElast_Opt <- data.frame(matrix(NA, ncol=26, nrow=100))
names(dElast_Opt) <- c("Industry", "de_KK", "de_KL", "de_KE", "de_KM", "de_KS",
                      "de_LK", "de_LL", "de_LE", "de_LM", "de_LS", "de_EK", "de_EL", "de_EE", "de_EM", "de_ES",
                      "de_MK", "de_ML", "de_ME", "de_MM", "de_MS", "de_SK", "de_SL", "de_SE", "de_SM", "de_SS")

for(i in c(1:100)){ #1:61
  data <- get(paste0("Data5.",i))
  ...
  for(j in c(1:nrow(data))){
    ...
    t_ed <- data$t[data$Year == (ann - 1) & data$Country == ctr & data$Industry == ind]
    ...
    try(c(
      ...
      data$t_ed[j] <- t_ed), silent = TRUE)# - data$lnY[j]), silent = TRUE)
  }
  ...
  inst1 <- ~lnK1 + lnL1 + lnE1 + lnM1 + lnY1 + t_ed + dK1
  inst2 <- ~lnK1 + lnL1 + lnE1 + lnM1 + lnY1 + t_ed + dL1
  inst3 <- ~lnK1 + lnL1 + lnE1 + lnM1 + lnY1 + t_ed + dE1
  inst4 <- ~lnK1 + lnL1 + lnE1 + lnM1 + lnY1 + t_ed + dM1
  ...
  try(c(system_eqn <- systemfit(eqlist, data = data, method="3SLS", inst = inst,
        ...
        R2 <- c((summary(system_eqn)[[10]][[1]])$r.squared, (summary(system_eqn)[[10]][[2]])$r.squared, 
                ...
                bgtest(eq1, order.by = data$Year, data=data, type = "Chisq")[4], bgtest(eq2, order.by = data$Year, data=data, type = "Chisq")[4],
                bgtest(eq3, order.by = data$Year, data=data, type = "Chisq")[4], bgtest(eq4, order.by = data$Year, data=data, type = "Chisq")[4]),
        success <- TRUE
  ), silent = TRUE)
  
  #system_eqn <- systemfit(eqlist, data = data, method="3SLS", inst = inst,
  #                        method3sls="GMM", restrict.matrix = restrict, maxiter = 100)
  ...
  if (success == FALSE | any(R2[1:4] < 0)){
    ...
    for (j in 1:8){
      R2_Opt[i,j+1] <- R2[j] #(summary(system_eqn)[[10]][[1]])$r.squared
    }
    
    for (j in 1:8){
      Coef_Opt[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,1]
      Coef_Opt[i,j+9] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,1]
      Coef_Opt[i,j+17] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,1]
      Coef_Opt[i,j+25] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,1]
      Err_Opt[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,2]
      Err_Opt[i,j+9] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,2]
      Err_Opt[i,j+17] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,2]
      Err_Opt[i,j+25] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,2]
      pval_Opt[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,4]
      pval_Opt[i,j+9] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,4]
      pval_Opt[i,j+17] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,4]
      pval_Opt[i,j+25] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,4]
    }
    ...
    Beta_KK <- (summary(system_eqn)[[10]][[1]])$coefficients[2] * (1 - Lambda)
    Beta_KL <- (summary(system_eqn)[[10]][[1]])$coefficients[3] * (1 - Lambda)
    Beta_KE <- (summary(system_eqn)[[10]][[1]])$coefficients[4] * (1 - Lambda)
    Beta_KM <- (summary(system_eqn)[[10]][[1]])$coefficients[5] * (1 - Lambda)
    Beta_KS <- - Beta_KK - Beta_KL - Beta_KE - Beta_KM
    
    Beta_LK <- Beta_KL
    Beta_LL <- (summary(system_eqn)[[10]][[2]])$coefficients[3] * (1 - Lambda)
    Beta_LE <- (summary(system_eqn)[[10]][[2]])$coefficients[4] * (1 - Lambda)
    Beta_LM <- (summary(system_eqn)[[10]][[2]])$coefficients[5] * (1 - Lambda)
    Beta_LS <- - Beta_LK - Beta_LL - Beta_LE - Beta_LM
    
    Beta_EK <- Beta_KE
    Beta_EL <- Beta_LE
    Beta_EE <- (summary(system_eqn)[[10]][[3]])$coefficients[4] * (1 - Lambda)
    Beta_EM <- (summary(system_eqn)[[10]][[3]])$coefficients[5] * (1 - Lambda)
    Beta_ES <- - Beta_EK - Beta_EL - Beta_EE - Beta_EM
    
    Beta_MK <- Beta_KM
    Beta_ML <- Beta_LM
    Beta_ME <- Beta_EM
    Beta_MM <- (summary(system_eqn)[[10]][[4]])$coefficients[5] * (1 - Lambda)
    ...
    Elast[1] <- (Beta_KK + SKav^2 - SKav) / SKav
    Elast[2] <- (Beta_KL + SKav * SLav) / SKav
    Elast[3] <- (Beta_KE + SKav * SEav) / SKav
    Elast[4] <- (Beta_KM + SKav * SMav) / SKav
    Elast[5] <- (Beta_KS + SKav * SSav) / SKav
    
    Elast[6] <- (Beta_LK + SLav * SKav) / SLav
    Elast[7] <- (Beta_LL + SLav^2 - SLav) / SLav
    Elast[8] <- (Beta_LE + SLav * SEav) / SLav
    Elast[9] <- (Beta_LM + SLav * SMav) / SLav
    Elast[10] <- (Beta_LS + SLav * SSav) / SLav
    
    Elast[11] <- (Beta_EK + SEav * SKav) / SEav
    Elast[12] <- (Beta_EL + SEav * SLav) / SEav
    Elast[13] <- (Beta_EE + SEav^2 - SEav) / SEav
    Elast[14] <- (Beta_EM + SEav * SMav) / SEav
    Elast[15] <- (Beta_ES + SEav * SSav) / SEav
    
    Elast[16] <- (Beta_MK + SMav * SKav) / SMav
    Elast[17] <- (Beta_ML + SMav * SLav) / SMav
    Elast[18] <- (Beta_ME + SMav * SEav) / SMav
    Elast[19] <- (Beta_MM + SMav^2 - SMav) / SMav
    Elast[20] <- (Beta_MS + SMav * SSav) / SMav
    
    Elast[21] <- (Beta_SK + SSav * SKav) / SSav
    Elast[22] <- (Beta_SL + SSav * SLav) / SSav
    Elast[23] <- (Beta_SE + SSav * SEav) / SSav
    Elast[24] <- (Beta_SM + SSav * SMav) / SSav
    Elast[25] <- (Beta_SS + SSav^2 - SSav) / SSav
    
    eLambda <- (summary(system_eqn)[[10]][[1]])$coefficients[8,2]
    
    eKK <- (summary(system_eqn)[[10]][[1]])$coefficients[2,2]
    dBeta_KK <- eKK + eKK * Lambda + eLambda * (summary(system_eqn)[[10]][[1]])$coefficients[2]
    eKL <- (summary(system_eqn)[[10]][[1]])$coefficients[3,2]
    dBeta_KL <- eKL + eKL * Lambda + eLambda * (summary(system_eqn)[[10]][[1]])$coefficients[3]
    eKE <- (summary(system_eqn)[[10]][[1]])$coefficients[4,2]
    dBeta_KE <- eKE + eKE * Lambda + eLambda * (summary(system_eqn)[[10]][[1]])$coefficients[4]
    eKM <- (summary(system_eqn)[[10]][[1]])$coefficients[5,2]
    dBeta_KM <- eKM + eKM * Lambda + eLambda * (summary(system_eqn)[[10]][[1]])$coefficients[5]
    dBeta_KS <- sqrt(dBeta_KK^2+dBeta_KL^2+dBeta_KE^2+dBeta_KM^2)
    
    dBeta_LK <- dBeta_KL
    eLL <- (summary(system_eqn)[[10]][[2]])$coefficients[3,2]
    dBeta_LL <- eLL + eLL * Lambda + eLambda * (summary(system_eqn)[[10]][[2]])$coefficients[3]
    eLE <- (summary(system_eqn)[[10]][[2]])$coefficients[4,2]
    dBeta_LE <- eLE + eLE * Lambda + eLambda * (summary(system_eqn)[[10]][[2]])$coefficients[4]
    eLM <- (summary(system_eqn)[[10]][[2]])$coefficients[5,2]
    dBeta_LM <- eLM + eLM * Lambda + eLambda * (summary(system_eqn)[[10]][[2]])$coefficients[5]
    dBeta_LS <- sqrt(dBeta_LK^2+dBeta_LL^2+dBeta_LE^2+dBeta_LM^2)
    
    dBeta_EK <- dBeta_KE
    dBeta_EL <- dBeta_LE
    eEE <- (summary(system_eqn)[[10]][[3]])$coefficients[4,2]
    dBeta_EE <- eEE + eEE * Lambda + eLambda * (summary(system_eqn)[[10]][[3]])$coefficients[4]
    eEM <- (summary(system_eqn)[[10]][[3]])$coefficients[5,2]
    dBeta_EM <- eEM + eEM * Lambda + eLambda * (summary(system_eqn)[[10]][[3]])$coefficients[5]
    ...
    dBeta_MM <- eMM + eMM * Lambda + eLambda * (summary(system_eqn)[[10]][[4]])$coefficients[5]
    ...
    for (j in 1:25){
      Elast_Opt[i,j+1] <- Elast[j]
      dElast_Opt[i,j+1] <- dElast[j]
    } 
  }
}

write.csv(R2_Opt, "R2_Opt.csv")
write.csv(Elast_Opt, "Elast_Opt.csv")
write.csv(dElast_Opt, "dElast_Opt.csv")

write.csv(Coef_Opt, "Coef_Opt.csv")
write.csv(Err_Opt, "Err_Opt.csv")
write.csv(pval_Opt, "pval_Opt.csv")

rm(list = ls()[grep("^Data5.", ls())])
rm(dlistopt)
