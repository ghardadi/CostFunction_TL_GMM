...
R2_4DPool <- data.frame(matrix(NA, ncol=13, nrow=160))
names(R2_4DPool) <- c("Industry", "R2_Eq1", "R2_Eq2", "R2_Eq3", "R2_Eq4", "Bg_Eq1", "Bg_Eq2",
                    "Bg_Eq3", "Bg_Eq4", "Mono", "Non-Mono", "Concavity", "Non-Concavity")

Coef_4DPool <- data.frame(matrix(NA, ncol=29, nrow=160))
names(Coef_4DPool) <- c("Industry",
...
Err_4DPool <- data.frame(matrix(NA, ncol=29, nrow=160))
names(Err_4DPool) <- c("Industry",
                       "eq1_(Intercept)", "eq1_lnK", "eq1_lnL", "eq1_lnE", "eq1_lnM", "eq1_lnY", "eq1_t",
                       "eq2_(Intercept)", "eq2_lnK", "eq2_lnL", "eq2_lnE", "eq2_lnM", "eq2_lnY", "eq2_t",
                       "eq3_(Intercept)", "eq3_lnK", "eq3_lnL", "eq3_lnE", "eq3_lnM", "eq3_lnY", "eq3_t",
                       "eq4_(Intercept)", "eq4_lnK", "eq4_lnL", "eq4_lnE", "eq4_lnM", "eq4_lnY", "eq4_t")

pval_4DPool <- data.frame(matrix(NA, ncol=29, nrow=160))
names(pval_4DPool) <- c("Industry",
                        "eq1_(Intercept)", "eq1_lnK", "eq1_lnL", "eq1_lnE", "eq1_lnM", "eq1_lnY", "eq1_t",
                        "eq2_(Intercept)", "eq2_lnK", "eq2_lnL", "eq2_lnE", "eq2_lnM", "eq2_lnY", "eq2_t",
                        "eq3_(Intercept)", "eq3_lnK", "eq3_lnL", "eq3_lnE", "eq3_lnM", "eq3_lnY", "eq3_t",
                        "eq4_(Intercept)", "eq4_lnK", "eq4_lnL", "eq4_lnE", "eq4_lnM", "eq4_lnY", "eq4_t")

GlobalTest_Pool <- data.frame(matrix(NA, ncol=17, nrow=0))
names(GlobalTest_Pool) <- c("Industry", "Country", "Year", "ActSK", "ActSL", "ActSE", "ActSM", "FitSK", "FitSL", "FitSE", "FitSM",
...
for(i in c(1:160)){ #1:61
  ...
  eq1 <- SK ~ lnK + lnL + lnE + lnM + lnY + t
  eq2 <- SL ~ lnK + lnL + lnE + lnM + lnY + t
  eq3 <- SE ~ lnK + lnL + lnE + lnM + lnY + t
  eq4 <- SM ~ lnK + lnL + lnE + lnM + lnY + t
  ...
  R2 <- c((summary(system_eqn)[[10]][[1]])$r.squared, (summary(system_eqn)[[10]][[2]])$r.squared, 
          ...
          pbgtest(SK ~ lnK + lnL + lnE + lnM + lnY + t, order.by = data$Year, data=data, type = "Chisq")[4],
          pbgtest(SL ~ lnK + lnL + lnE + lnM + lnY + t, order.by = data$Year, data=data, type = "Chisq")[4],
          pbgtest(SE ~ lnK + lnL + lnE + lnM + lnY + t, order.by = data$Year, data=data, type = "Chisq")[4],
          pbgtest(SM ~ lnK + lnL + lnE + lnM + lnY + t, order.by = data$Year, data=data, type = "Chisq")[4])
  ...
  if (any(R2[1:5] < 0)){
    ...
    for (j in 1:8){
      R2_4DPool[i,j+1] <- R2[j] #(summary(system_eqn)[[10]][[1]])$r.squared
    }
    
    for (j in 1:7){
      Coef_4DPool[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,1]
      Coef_4DPool[i,j+8] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,1]
      Coef_4DPool[i,j+15] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,1]
      Coef_4DPool[i,j+22] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,1]
      Err_4DPool[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,2]
      Err_4DPool[i,j+8] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,2]
      Err_4DPool[i,j+15] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,2]
      Err_4DPool[i,j+22] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,2]
      pval_4DPool[i,j+1] <- (summary(system_eqn)[[10]][[1]])$coefficients[j,4]
      pval_4DPool[i,j+8] <- (summary(system_eqn)[[10]][[2]])$coefficients[j,4]
      pval_4DPool[i,j+15] <- (summary(system_eqn)[[10]][[3]])$coefficients[j,4]
      pval_4DPool[i,j+22] <- (summary(system_eqn)[[10]][[4]])$coefficients[j,4]
    }
    ...
    R2_4DPool[i,10] <- Ntrue
    R2_4DPool[i,11] <- Nfalse
    ...
    R2_4DPool[i,12] <- Ntrue
    R2_4DPool[i,13] <- Nfalse
    ...
    GlobalTest_Pool <- rbind(GlobalTest_Pool, test)
  }
}

write.csv(R2_4DPool, "R2_4DPool.csv")

write.csv(Coef_4DPool, "Coef_4DPool.csv")
write.csv(Err_4DPool, "Err_4DPool.csv")
write.csv(pval_4DPool, "pval_4DPool.csv")
